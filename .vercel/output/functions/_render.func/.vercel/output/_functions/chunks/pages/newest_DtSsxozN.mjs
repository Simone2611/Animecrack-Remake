...
const $$Newest = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$Newest;
  const { JSDOM } = jsdom;
  let arr = [];
  let genere = Astro2.request.url;
  let generefix = genere.replaceAll(Astro2.url.origin + "/newest", "");
  let generefix2 = generefix.replaceAll(/\d+/g, "");
  generefix2.replaceAll("?page=", "");
  await axios.get("https://www.animeworld.so/newest" + generefix).then(({ data }) => {
