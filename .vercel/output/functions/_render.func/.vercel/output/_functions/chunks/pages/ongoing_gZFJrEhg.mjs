...
const $$Ongoing = createComponent(async ($$result, $$props, $$slots) => {
  ...
  let generefix = genere.replaceAll(Astro2.url.origin + "/ongoing", "");
  let generefix2 = generefix.replaceAll(/\d+/g, "");
  generefix2.replaceAll("?page=", "");
  let arr = [];
  await axios.get("https://www.animeworld.so/ongoing" + generefix).then(({ data }) => {
