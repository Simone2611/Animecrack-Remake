...
const $$Updated = createComponent(async ($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$Updated;
  const { JSDOM } = jsdom;
  let genere = Astro2.request.url;
  let generefix = genere.replaceAll(Astro2.url.origin + "/updated", "");
  let generefix2 = generefix.replaceAll(/\d+/g, "");
  generefix2.replaceAll("?page=", "");
  let arr = [];
  await axios.get("https://www.animeworld.so/updated" + generefix).then(({ data }) => {
